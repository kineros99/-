-- tools/seed_from_array.sql
-- Este script insere as lojas do arquivo array_lojas_zonaSul.txt.
-- As coordenadas e bairros foram pré-calculados usando a API OpenCage.

-- Primeiro, cria um usuário genérico para ser o "dono" destas lojas importadas.
INSERT INTO users (username, role) VALUES ('import_zonasul', 'merchant') ON CONFLICT (username) DO NOTHING;

-- Insere as lojas, associando-as ao usuário recém-criado.
-- A cláusula ON CONFLICT previne a inserção de lojas duplicadas caso o script seja executado mais de uma vez.
